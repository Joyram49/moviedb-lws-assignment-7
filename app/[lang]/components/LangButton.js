...
export default function LangButton() {
  ...
  const handleClick = (e) => {
    ...
    const indexNum = pathname.indexOf(lang);
    if (lang === "en") {
      pathname[indexNum] = "bn";
      const newPathname = pathname.join("/");
      window.location.pathname = newPathname;
    } else {
      pathname[indexNum] = "en";
      const newPathname = pathname.join("/");
      window.location.pathname = newPathname;
    }
  };
  ...
  return (
    <button
      className='bg-primary/20 dark:bg-primary/[7%] rounded-lg backdrop-blur-[2px] p-1 flex justify-center items-center gap-x-1'
      onClick={handleClick}
    >
      {lang === "en" ? (
        <svg
      ...
      <p className='text-primary font-medium'>{lang === "en" ? "BN" : "EN"}</p>
    </button>
  );
}
